var app = angular.module('essaydiarydetails',[]);
...
app.controller('essaydiarydatas', function($scope,$http) {
    /**初始化基本信息内容*/
    $scope.getcontentessay=function(){
        $http({
            method:'post',
            url:'/essaydiary/getTheessaydiaryDataDetail',
            data:$.param({
            ...
            function success(resq){
                if(resq.data.success){
                    $scope.e_title = resq.data.data.rows[0].e_title;
                    $scope.e_readnumber = resq.data.data.rows[0].e_readnumber;
                    $scope.e_addtime =resq.data.data.rows[0].e_addtime;
                    $scope.u_truename =resq.data.data.rows[0].userinfo.u_truename;
                    $scope.e_content =  resq.data.data.rows[0].e_content;
