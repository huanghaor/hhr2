var app = angular.module('publishDonkeyTrip',[]);
//获取导航栏列表
...
//编辑器
app.controller('publishDonkey', function($scope,$http) {
    ...
    //上传图片
    editor.customConfig.uploadImgServer = '/donkeytrip/uploadPicture';
    ...
    $scope.submitArticleFormData = function () {
        $scope.t_tourism_strategy=editor.txt.html();
        $scope.uploadFileData={
            t_tourism_strategy:$scope.t_tourism_strategy,
            t_title:$scope.t_title,
            t_begin_time:$scope.t_begin_time,
            t_end_time:$scope.t_end_time,
            t_travelperson:$scope.t_travelperson,
            t_place:$scope.t_place,
            t_per_capita_consumption:$scope.t_per_capita_consumption,
        ...
        if ($scope.signUpForm.$invalid){
            ...
            $http({
                method:'post',
                url:'/donkeytrip/publishDonkeytripData',
                data:$.param( $scope.uploadFileData),
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                }
            }).then(
    ...
    $("#test-upload").fileinput({
        language: 'zh', //设置语言
        uploadUrl: '/donkeytrip/saveTheFile', // you must set a valid URL here else you will get an error
