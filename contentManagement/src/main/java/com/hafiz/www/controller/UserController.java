...
public class UserController {
    ...
    public JsonResult judgeIsLogin(){
        ...
        if(SessionUtils.isLoggedIn()){//已经登陆
           Userinfo userinfo= userService.selectByPrimaryKey(SessionUtils.getLoginUserId());
            ...
            jsonResult = jsonResult.newInstanceFail("/file/systemData/20181226.png");
        }
        return jsonResult;
    }
    /**
    ...
    public JsonResult Register(Userinfo userinfo,HttpServletRequest request){
        ...
        if(userService.selectByEmail(userinfo.getU_email()).size()>0){
            ...
            if ((userinfo.getCode()).toLowerCase().equals((code.toString()).toLowerCase())) {
    ...
    public JsonResult UpdataUserinfo(Userinfo userinfo){
        ...
        if(!SessionUtils.isLoggedIn()){
            ...
            try {
                ...
                if(id==1){
                    SessionUtils.logout();//当当前用户修改信息后退出登陆
                    jsonResult = jsonResult.newInstanceSucess_d1("修改成功，请重新登陆！");
                }else{
     ...
     * @return
     */
    @RequestMapping(value = "/updateUserinfoByManagemer",method = RequestMethod.POST)
    @ResponseBody
    public JsonResult UpdataUserinfoByManagemer(Userinfo userinfo){
        JsonResult jsonResult = new JsonResult();
        if(!SessionUtils.isLoggedIn()){
            return JsonResult.newInstanceFail("请登录系统！");
        }else {
     ...
     * @return
     */
    @RequestMapping(value = "/deleteUserinfoByManagemer",method = RequestMethod.POST)
    @ResponseBody
    public JsonResult deleteUserinfoByManagemer(Userinfo userinfo){
        JsonResult jsonResult = new JsonResult();
        if(!SessionUtils.isLoggedIn()){
            return JsonResult.newInstanceFail("请登录系统！");
        }else {
            try {
                int id = userService.deleteUserinfoByManagemer(userinfo.getU_id());
                if(id==1){
                    jsonResult = jsonResult.newInstanceSucess_d1("删除成功！");
                }else{
                    jsonResult = jsonResult.newInstanceFail("删除失败，请稍后再试");
                }
            }catch (Exception e){
                e.printStackTrace();
                jsonResult = jsonResult.newInstanceFail("系统出错，请稍后再试");
    ...
    public ModelAndView updateTheUser_head(MultipartFile u_userhead){
        ...
        String type="userhead";
        String head_url = saveThePicture.savePicture(u_userhead,type);
    ...
    public JsonResult getAllUsers(Userinfo userinfo){
        JsonResult jsonResult = new JsonResult();
        jsonResult=userService.getAllUsers(userinfo);
        return jsonResult;
    }
